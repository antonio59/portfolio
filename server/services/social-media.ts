...
// Post to Twitter
async function postToTwitter(post: SocialMediaPost): Promise<{success: boolean, message?: string}> {
  if (!currentConfig.twitter.enabled) {
    return { success: false, message: "Twitter posting is disabled" };
  }
  
  // Check for required credentials
  if (!currentConfig.twitter.apiKey || 
  ...
  try {
    ...
    console.log("Posted to Twitter:", formatPost(post, currentConfig.defaultFormat));
    ...
    return { 
      success: false, 
      message: error instanceof Error ? error.message : "Unknown error posting to Twitter" 
    };
  }
}

// Post to Bluesky
async function postToBluesky(post: SocialMediaPost): Promise<{success: boolean, message?: string}> {
  ...
  try {
    ...
    // For this example, we'll just simulate a successful post
    console.log("Posted to Bluesky:", formatPost(post, currentConfig.defaultFormat));
    ...
    return { 
      success: false, 
      message: error instanceof Error ? error.message : "Unknown error posting to Bluesky" 
    };
  }
}
...
export function updateSocialMediaConfig(config: Partial<SocialMediaConfig>): SocialMediaConfig {
  currentConfig = {
    ...currentConfig,
    ...config,
    twitter: {
