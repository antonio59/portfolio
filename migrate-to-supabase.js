...
if (!process.env.DATABASE_URL) {
  console.error(
    "\x1b[31mERROR: DATABASE_URL environment variable is not set.\x1b[0m",
  );
  console.log(
    "\x1b[33mPlease follow these steps to get your Supabase PostgreSQL URL:\x1b[0m",
  );
  console.log(
  ...
  console.log(
    '  6. Run this command: \x1b[36mexport DATABASE_URL="your_url_here"\x1b[0m',
  );
  console.log(
    "  7. Then run this script again: \x1b[36mnode migrate-to-supabase.js\x1b[0m",
  );
  process.exit(1);
}
...
async function runMigration() {
  try {
    ...
    console.log(
      "\x1b[33m1. Setting storage implementation to Supabase...\x1b[0m",
    );

    // Ensure the server/supabase-storage.ts file exists
    const supabaseStoragePath = path.join(
      __dirname,
      "server",
      "supabase-storage.ts",
    );
    if (!fs.existsSync(supabaseStoragePath)) {
      console.error(
        "\x1b[31msupabase-storage.ts file not found. Cannot continue migration.\x1b[0m",
      );
      process.exit(1);
    }
    ...
    if (
      ...
      console.log(
        "\x1b[32mAlready using Supabase storage implementation.\x1b[0m",
      );
    } else {
      ...
      console.log(
        "\x1b[32mUpdated storage.ts to use Supabase implementation.\x1b[0m",
      );
    }
    ...
    try {
      ...
      console.log(
        "\x1b[32mDatabase schema migration completed successfully.\x1b[0m",
      );
    } catch (error) {
      console.error(
        "\x1b[31mError running database migrations:\x1b[0m",
        error.message,
      );
      console.log(
        '\x1b[33mPlease fix the issues and run "npm run db:push" manually.\x1b[0m',
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (fs.existsSync(path.join(__dirname, "server", "seed.ts"))) {
        ...
        console.log(
          "\x1b[33mNo seed script found. Skipping data seeding.\x1b[0m",
        );
      }
    ...
    try {
      ...
      console.error(
        "\x1b[31mDatabase connection test failed:\x1b[0m",
        error.message,
      );
      process.exit(1);
    }
...
`;
    ...
    console.log(
      "\x1b[36m=== Migration to Supabase Completed Successfully ===\x1b[0m",
    );
    console.log("\x1b[33mNext steps:\x1b[0m");
    console.log(
      "1. Make sure your Supabase project has a secure password and restricted access",
    );
    console.log(
      "2. Set up the DATABASE_URL environment variable in your Netlify project settings",
    );
    console.log(
      '3. Deploy your application to Netlify with the "netlify deploy" command',
    );
  } catch (error) {
