...
import { AlertCircle, CheckCircle, Edit, Trash2, Plus, FileText } from 'lucide-react';
...
export function CaseStudyManager() {
  ...
  const createCaseStudyMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest('POST', '/api/admin/blog/case-studies', data);
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/admin/blog/case-studies'] });
    },
    onError: (error: Error) => {
      toast({
  ...
  const updateCaseStudyMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const res = await apiRequest('PUT', `/api/admin/blog/case-studies/${id}`, data);
      return await res.json();
    },
    onSuccess: () => {
      toast({
  ...
  const deleteCaseStudyMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await apiRequest('DELETE', `/api/admin/blog/case-studies/${id}`);
      return await res.json();
    },
    onSuccess: () => {
      toast({
  ...
  if (isLoadingBlogPosts || isLoadingCaseStudies) {
    return <div className="flex justify-center py-8">Loading case study data...</div>;
  }
    ...
    <div className="space-y-6">
      <Tabs defaultValue="existing" className="w-full">
          ...
          <div className="grid gap-4">
            {caseStudies && caseStudies.length > 0 ? (
                    ...
                    return (
                      <TableRow key={caseStudy.id}>
                        <TableCell className="font-medium">
                          {blogPost?.title || `Blog Post #${caseStudy.blogPostId}`}
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline">{caseStudy.projectType}</Badge>
                        </TableCell>
                        <TableCell>
                          {caseStudy.clientName || 'N/A'}
                        </TableCell>
              ...
              <Alert>
                ...
                <AlertDescription>
                  You haven't added any case studies yet. Go to the "Add New Case Study" tab to create one.
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        <TabsContent value="add" className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Select a blog post to add detailed case study information. Only blog posts without existing case studies are shown.
            </AlertDescription>
          </Alert>
          ...
          {eligibleBlogPosts.length > 0 ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                All your blog posts already have case studies attached. Create a new blog post if you want to add more case studies.
              </AlertDescription>
            </Alert>
          )}
        </TabsContent>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Add Case Study for "{selectedBlogPost?.title}"</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Edit Case Study for "
              {getBlogPostForCaseStudy(selectedCaseStudy?.blogPostId || 0)?.title || 'Blog Post'}"
            </DialogTitle>
            <DialogDescription>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {getBlogPostForCaseStudy(selectedCaseStudy?.blogPostId || 0)?.title || 'this blog post'}"?
