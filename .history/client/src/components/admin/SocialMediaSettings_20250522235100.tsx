...
type FormValues = SocialMediaSettings;
...
export default function SocialMediaSettings() {
  ...
  const form = useForm<FormValues>({
    // @ts-expect-error - Known issue with zodResolver types
  ...
  const testMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/admin/social-media/test");
      return response.json();
    },
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Social media connection test has been completed.",
      });
    },
    onError: (error) => {
      toast({
  ...
  const handleTest = async () => {
    try {
      const response = await fetch('/api/admin/social-media/test', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to test social media settings');
      }

      const result = await response.json();
      toast({
        title: 'Test successful',
        description: 'Social media settings are working correctly.',
      });
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'Failed to test social media settings. Please check your configuration.',
        variant: 'destructive',
      });
    }
  };
